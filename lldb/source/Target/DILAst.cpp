    ...
    std::shared_ptr<ExecutionContextScope> ctx_scope)
{
  ...
  if (type_name && type_name[0] && target_sp) {
    ...
    images.FindTypes(nullptr, const_type_name, exact_match, UINT32_MAX,
                     searched_symbol_files, type_list);

    for (size_t idx = 0; idx < type_list.GetSize(); idx++) {
      lldb::TypeSP type_sp(type_list.GetTypeAtIndex(idx));
      if (type_sp)
    ...
    CompilerType *scope_ptr)
{
  ...
  if (!global_scope && !name_ref.contains("::")) {
    if (!scope_ptr || !scope_ptr->IsValid()) {
      ...
      lldb::ValueObjectSP value_sp = frame->FindVariable(ConstString(name_ref));
